...
public class PlayerService {
    ...
    public List<Player> showSortedList(HttpServletRequest request) {
        ...
        PlayerOrder order = request.getParameter("order") != null ? PlayerOrder.valueOf(request.getParameter("order")) : PlayerOrder.ID;
    ...
    public Integer filterPlayers(HttpServletRequest request) {
    ...
    public boolean isPlayerDel(String id) {
        long playerId = Long.parseLong(id);
        if (isPlayerExist(playerId)) {
            playerRepository.deletePlayersById(playerId);
    ...
    private String queryBuilderByPlayerData(HttpServletRequest request) {
        ...
        if (request.getParameter("name") != null) {
            sb.append("AND name LIKE ").append(":name ");
        }
        if (request.getParameter("title") != null) {
            sb.append("AND title LIKE ").append(":title ");
        }
        if (request.getParameter("race") != null) {
            sb.append("AND race = ").append(":race ");
        }
        if (request.getParameter("banned") != null) {
            sb.append("AND banned = :banned ");
        }
        if (request.getParameter("profession") != null) {
            sb.append("AND profession = ").append(":profession ");
        }
        if (request.getParameter("before") != null) {
            sb.append("AND birthday <= ").append(":before ");
        }
        if (request.getParameter("after") != null) {
            sb.append("AND birthday >= ").append(":after ");
        }
        if (request.getParameter("minExperience") != null) {
            sb.append("AND experience >= ").append(":minExperience ");
        }
        if (request.getParameter("maxExperience") != null) {
            sb.append("AND experience <= ").append(":maxExperience ");
        }
        if (request.getParameter("minLevel") != null) {
            sb.append("AND level >= ").append(":minLevel ");
        }
        if (request.getParameter("maxLevel") != null) {
            sb.append("AND level <= ").append(":maxLevel ");
        }
        return sb.toString();
    }
    ...
    public boolean isPlayerExist(Long id) {
        return playerRepository.existsPlayerById(id);
    }
    ...
    public boolean checkPlayer(Player player) {
        if (player.getName() == null || player.getName().length() > 12
                ...
                || player.getExperience() > 10_000_000 || player.getBirthday().getTime() < 0
        ) {
            return false;
        }
    ...
    public ResponseEntity<Player> updateActualPlayer(String id, Player incomingPlayer) {
        Optional<Player> optionalPlayer = findPlayerById(id);

        if (optionalPlayer.isPresent()) {
            Player playerForUpdate = optionalPlayer.get();
            if (updateActualTitle(incomingPlayer, playerForUpdate)) {
                return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
            }
            if (updatePlayerExperience(incomingPlayer, playerForUpdate)) {
                return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
            }
            if (updateActualName(incomingPlayer, playerForUpdate)) {
                return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
            }
            if (updateActualBirthday(incomingPlayer, playerForUpdate)) {
                return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
            }

            updateActualRace(incomingPlayer, playerForUpdate);
            updateActualProfession(incomingPlayer, playerForUpdate);

            updatePlayerBan(incomingPlayer, playerForUpdate);

            setCurrentLevel(playerForUpdate);
            playerRepository.saveAndFlush(playerForUpdate);
    ...
    private boolean updateActualBirthday(Player incomingPlayer, Player playerForUpdate) {
    ...
    private void updateActualProfession(Player incomingPlayer, Player playerForUpdate) {
        if (incomingPlayer.getProfession() != null) {
            playerForUpdate.setProfession(incomingPlayer.getProfession());
        }
    }

    private void updateActualRace(Player incomingPlayer, Player playerForUpdate) {
        if (incomingPlayer.getRace() != null) {
            playerForUpdate.setRace(incomingPlayer.getRace());
        }
    }

    private boolean updateActualTitle(Player incomingPlayer, Player playerForUpdate) {
    ...
    private boolean updateActualName(Player incomingPlayer, Player playerForUpdate) {
    ...
    private List<Player> getPlayersFromQuery(HttpServletRequest request) {
        ...
        try {
            ...
            if (request.getParameter("before") != null) {
                String dateText = toDateFormat(request.getParameter("before"));
                query.setParameter("before", dateText);
            }
            if (request.getParameter("after") != null) {
                String dateText = toDateFormat(request.getParameter("after"));
    ...
    private String toDateFormat(String dataFromRequest) {
